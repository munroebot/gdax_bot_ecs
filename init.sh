...
x=$(aws ssm get-parameter --region us-west-2 --with-decryption --name gdax_bot_settings_conf);
echo -e $x > /opt/gdax_bot/settings.conf

# Fetch task runner script from paramter store
j=$(aws ssm get-parameter  --region us-west-2 --name gdaxbot_trades_$INTERVAL.sh);
